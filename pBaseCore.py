...
class Datas():
	...
	def get_NumberPoly(self):
		if not self.odd:
			return np.arange(0, self.lmax + 1, 2).shape[0]
		else:
			return np.arange(0, self.lmax + 1, 1).shape[0]
	...
	def OpenFile(self, filepath):
		if filepath[-5:] == '.fits':
			...
			print("Incorrect data file format")

	def SaveFileFits(self, filepath):
		hdu_pes = fits.ImageHDU(np.array([np.arange(self.normed_pes.shape[0]), self.normed_pes, self.pes_error]))
		hdu_ang = fits.ImageHDU(self.ang)
		hdu_ang_var = fits.ImageHDU(self.ang_var)
		hdu_out = fits.PrimaryHDU(self.output)
		col1 = fits.Column(name='Max Legendre', format='I', array=np.array([self.lmax]))
		col2 = fits.Column(name='Image Center', format='2I', array=np.array(self.center))
		col3 = fits.Column(name='Selection radius', format='I', array=np.array([self.r]))
		col4 = fits.Column(name='odd', format='I', array=np.array([self.odd]))
		tbhdu = fits.BinTableHDU.from_columns([col1, col4, col2, col3])  # , tbtype='TableHDU')
		hdulist = fits.HDUList([hdu_out, hdu_pes, hdu_ang, hdu_ang_var, tbhdu])
	...
	def LoadBasis(self, path):
		...
		if not self.odd:
			filenamescore = ''.join(['P%i' % i for i in np.arange(0, self.lmax + 1, 2)])
		else:
			filenamescore = ''.join(['P%i' % i for i in np.arange(0, self.lmax + 1)])
		fileextension = '.dat'
	...
	def Invert(self, polar, u):
		...
		self.coefficients, residuts, siz, singulars = np.linalg.lstsq(u, polar[
			0])  # Need to build up angular distribution and PES
	...
	def image_for_display(self):
		...
		if 2 * dim + 1 < min(self.raw.shape):
			...
			self.output[self.center[1] - dim:self.center[1] + dim + 1,
			self.center[0] - dim:self.center[0] + dim + 1] = outdata.reshape((2 * dim + 1, 2 * dim + 1))
		else:
